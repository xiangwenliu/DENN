...
m = np.array([0.54811931,  1.0414411,  1.43503981,  1.76987688,  2.06975004,
              2.33691141,  2.59669649,  2.82211753,  3.05080109])
v = np.array([0.692952,  0.50219263,  0.37190962,  0.29367094,  0.24688417,
              0.20508072,  0.18324195,  0.14794321,  0.14100618])
...
def prelu(_x, name='prelu'):
  with tf.variable_scope(name) as vs:
    ...
    alphas = tf.get_variable('alpha', shape=_x.get_shape()[1:],
...
class MyDenseLayer(Layer):
    ...
    def __init__(
            ...
            n_units=100,
            act=tf.identity,
            W_init=tf.uniform_unit_scaling_initializer(factor=0.1),
        ...
        with tf.variable_scope(name) as vs:
            # get the mask
            mm = get_mask(self.branch, self.n_in, n_units)
            mask = tf.get_variable(name='mask', shape=[branch * n_units, self.n_in], trainable=False,
                    initializer=tf.constant_initializer(value=mm))

            W = tf.get_variable(name='W', shape=[self.n_units*self.branch, self.n_in],
            ...
            # lay = int(name[3:])
            # if is_train is True and keep_prob < 1.0 and lay > 1:
