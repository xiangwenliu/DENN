...
def cal(width, depth, scale, branch, acc, times, eps, batchsize):
  with tf.device('/gpu'):
    ...
    h1_value = model_feedforward(data_x, width, depth, scale)
    ...
    for i in range(times):
      ...
      for j in range(acc):
        ...
        if j == 0:
          ...
          h1_value_pre = np.sign(h1_value_pre.reshape([depth,
                                                       batchsize, width]))
        else:
          ...
          h1_value_now = np.sign(h1_value_now.reshape([depth,
                                                       batchsize, width]))
          
          cnt += np.average(np.sum(h1_value_pre != h1_value_now, axis=-1),
                            axis=-1)
          h1_value_pre = h1_value_now
